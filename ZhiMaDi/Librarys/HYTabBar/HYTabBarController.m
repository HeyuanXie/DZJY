...
#pragma mark -  @implementation HYTabBarController

@interface HYTabBarController()<TabBarViewDelegate>
...
///单例
static HYTabBarController* _instance = nil;
...
- (id)initWithTabBarSelectedImages:(NSMutableArray *)selected
    ...
    if ([super init]) {
        ...
        self.centerItem = self.tabBarView.centerButton;
    }
    return self;
}
...
-(void)tabBarViewCenterItemClick:(UIButton *)button{
    if([self respondsToSelector:@selector(centerBtnClickBlock)]) {
...
-(void)showCenterViewController:(BOOL)show animated:(BOOL)animated{
    ...
    [self hyTabBarHidden:NO animated:YES];
...
-(void)layoutSubviews{
    ...
    for (int i = 0; i < self.buttonArray.count;i ++) {
        ...
        if (_showCenter && button.tag == count/2) {
            buttonX += centerButtonWH + padding;
        }
